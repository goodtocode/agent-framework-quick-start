...
public class InjectUserContextBehavior<TRequest>(IClaimsReader claimsReader) : IPipelineBehavior<TRequest>
...
/// <param name="claimsReader">Service for reading claims from HTTP authentication context</param>
public class InjectUserContextBehavior<TRequest, TResponse>(IClaimsReader claimsReader) : IPipelineBehavior<TRequest, TResponse>
       where TRequest : IRequiresUserContext
{
    /// <summary>
    /// Processes the request by injecting user context before invoking the next handler.
