...
namespace Goodtocode.AgentFramework.Tests.Integration.ChatCompletion
{
    ...
    public class GetMyChatSessionsPaginatedQueryStepDefinitions : TestBase
    {
        ...
        public async Task WhenIGetTheChatSessionsPaginated()
        {
            if (_exists)
            {
                var chatSession = ChatSessionEntity.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Session", ChatMessageRole.assistant, "First Message", "First Response", userContext.OwnerId, userContext.TenantId);
            ...
            if (validationResponse.IsValid)
                try
                {
                    var handler = new GetMyChatSessionsPaginatedQueryHandler(context);
