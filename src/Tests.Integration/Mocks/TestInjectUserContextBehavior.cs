...
internal sealed class TestInjectUserContextBehavior<TRequest>(IClaimsReader claimsReader) : IPipelineBehavior<TRequest>
       where TRequest : IRequiresUserContext
{
    ...
    {
        request.UserContext = UserContext.Create(claimsReader);
        await nextInvoker();
    }
}
...
/// <param name="claimsReader">Service for reading claims from HTTP authentication context</param>
public class TestInjectUserContextBehavior<TRequest, TResponse>(IClaimsReader claimsReader) : IPipelineBehavior<TRequest, TResponse>
       where TRequest : IRequiresUserContext
{
    /// <summary>
    /// Processes the request by injecting user context before invoking the next handler.
